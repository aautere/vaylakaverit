...
| PWA                             | React with Vite and a PWA plugin                                              | A static, iPhone-first SPA hosted through Azure Storage Static Website so every resource can remain in Sweden Central.                              |
...
| Authentication                  | Sign in with Apple OAuth handled by the API, plus device-bound guest sessions | The backend owns account linking, score authorization, and deletion. Apple requires a custom identity flow.                                         |
