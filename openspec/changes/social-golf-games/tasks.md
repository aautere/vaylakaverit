...
- [x] 2.1 Create the Väyläkaverit iPhone-first PWA foundation, including installable web-app
      metadata, responsive mobile layout, environment configuration, and local development
      instructions.
- [x] 2.2 Define Azure infrastructure with Bicep: Azure Storage Static Website, Azure Functions Flex
...
- [x] 2.3 Create the Azure Functions API foundation with persistent storage, authenticated API
...
- [x] 2.6 Add the verified Golf Talma Master course configuration and validate its hole and tee
...
- [x] 3.2 Implement short-lived opaque QR invitations, QR scanning on iPhone, two-to-four player
      joining, expiration/revocation, and full-round error handling.
- [x] 3.3 Build the shared round lobby so participants can confirm players, tees, and handicap
      readiness before games begin.

## 4. Build score and game logic

- [x] 4.1 Implement round-scoped score entry for a participant's own scores and score corrections
      for prior holes, with authorization and revision/conflict handling.
- [x] 4.2 Implement the deterministic scratch and handicap game evaluator, including gross/net
      score calculation, the shared multi-player hole outcome, and recomputation after corrections.
- [x] 4.3 Implement concurrent game setup for full rounds and selected upcoming hole ranges,
      including participant selection, optional reward text, scratch/handicap mode, and game history.
- [x] 4.4 Implement per-game tied-hole rules: no winner or carry-forward, plus the group-selected
      participants eligible to resolve a carried win.
- [x] 4.5 Implement per-game end-tie rules: recorded draw or one-hole-at-a-time continuation until
...
- [x] 5.1 Publish authoritative score and game-state updates through Azure Web PubSub to every
...
- [x] 5.2 Build the iPhone round view with per-hole score entry, live hole results, game standings,
      pending/reconnecting state, and correction feedback.
- [x] 5.3 Build the completed-round and completed-game history view with settings, participants,
      hole results, reward text, and final outcome.
- [x] 5.4 Build the Azure-independent local preview mode with an in-memory API store, local guest
...
- [ ] 6.3 Exercise a two-to-four player round on iPhone-sized Safari viewports, including PWA
      ...
      and history review.
- [x] 6.4 Run the selected project's formatting, type checking, tests, security checks, and
