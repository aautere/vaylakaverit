...
4. The system applies the selected configuration's default tee, updates the labelled tee choices,
   and preserves only valid field values.
5. The creator completes the existing round, player, and main-game settings and creates the lobby.
6. In a Rock Golf 18-hole round, later score and game context uses the round-hole number first and
...
- Labels, instructions, and error communication: course and length use native radio groups with
  ...
  relies on an icon or colour. Validation error text is adjacent to the relevant group and announced
  assertively.
...
- **Open question:** Rock Golf's official source must confirm its playable tee labels, default tee,
  handicap indexes, and 9/18-hole handicap data. This is a data gate in the technical design, not a
